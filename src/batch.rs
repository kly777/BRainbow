...
impl BatchResponse {
    pub fn all_ok(count: usize) -> Self {
        Self { ok: true, processed: count, succeeded: count, failed: 0, errors: None }
    }

    pub fn from_results(errors: Vec<BatchErrorDetail>, total: usize) -> Self {
        let failed = errors.len();
        Self { ok: failed == 0, processed: total, succeeded: total - failed, failed, errors: if failed > 0 { Some(errors) } else { None } }
    }

    pub fn empty() -> Self { Self::all_ok(0) }
}

impl<T: Serialize> BatchDataResponse<T> {
    pub fn from_results(items: Vec<T>, errors: Vec<BatchErrorDetail>, total: usize) -> Self {
        let failed = errors.len();
        Self { ok: failed == 0, processed: total, succeeded: total - failed, failed, items, errors: if failed > 0 { Some(errors) } else { None } }
    }

    pub fn all_ok(items: Vec<T>) -> Self {
        let count = items.len();
        Self { ok: true, processed: count, succeeded: count, failed: 0, items, errors: None }
    }

    pub fn empty() -> Self {
        Self { ok: true, processed: 0, succeeded: 0, failed: 0, items: vec![], errors: None }
    }
}
    ...
    Fut: std::future::Future<Output = Result<T, E>>,
{
    ...
    for (index, item) in items.into_iter().enumerate() {
        match f(item).await {
            ...
            Err(e) => errors.push(BatchErrorDetail { index, code: String::new(), message: e.to_string() }),
        }
    }
...
impl From<&crate::error::ServiceError> for BatchErrorDetail {
    fn from(err: &crate::error::ServiceError) -> Self {
        let code = err.status_code().canonical_reason().unwrap_or("Error").to_string();
        Self { index: 0, code, message: err.to_string() }
    }
}
    ...
    Fut: std::future::Future<Output = Result<T, (&'static str, String)>>,
{
    ...
    for (index, item) in items.into_iter().enumerate() {
        match f(item).await {
            Ok(data) => results.push(data),
            Err((code, message)) => errors.push(BatchErrorDetail { index, code: code.to_string(), message }),
        }
    }
...
mod tests {
    ...
    fn batch_response_partial_failure() {
        let errors = vec![
            BatchErrorDetail { index: 0, code: "Not Found".into(), message: "ID 1 不存在".into() },
            BatchErrorDetail { index: 2, code: "Internal Server Error".into(), message: "ID 3 约束冲突".into() },
        ];
    ...
    async fn batch_execute_all_success() {
        async fn double(x: i32) -> Result<i32, String> { Ok(x * 2) }
    ...
    async fn batch_execute_partial_failure() {
        async fn maybe_fail(x: i32) -> Result<i32, String> {
            if x == 2 { Err(format!("{} 出错了", x)) } else { Ok(x * 10) }
        }
        let (results, errors): (Vec<i32>, Vec<BatchErrorDetail>) = batch_execute(vec![1, 2, 3], maybe_fail).await;
