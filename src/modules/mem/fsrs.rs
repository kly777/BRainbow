...
pub fn schedule(
    s_old: f64, d_old: f64, state: CardState, step_index: Option<usize>,
    ...
    if state == Learning || state == New {
        let step = if state == New { 0 } else { step_index.unwrap_or(0) };
        return match rating {
            1 => {
...
pub fn preview(s_old: f64, d_old: f64, state: CardState, step_index: Option<usize>) -> [f64; 4] {
    ...
    if state == Relearning {
        ...
        return [
            ...
            if step + 1 >= RELEARN_STEPS.len() { compute_next(mem, 3) } else { RELEARN_STEPS[step + 1] as f64 },
            if step + 1 >= RELEARN_STEPS.len() { compute_next(mem, 4) } else { RELEARN_STEPS[step + 1] as f64 },
        ];
    }
