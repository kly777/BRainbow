...
pub struct UserService {
    repo: UserRepository,
    /// JWT 有效期（秒）：来自 env JWT_TTL_SECS 的静态启动配置；
    /// 密钥因支持 admin 运行时轮换而保持按调用传入，TTL 则无此需求
    jwt_ttl_secs: i64,
}

impl UserService {
    pub fn new(db: Arc<sqlx::SqlitePool>, jwt_ttl_secs: i64) -> Self {
        Self {
    ...
    pub async fn register(
        ...
        let token = create_token(user.id, &user.role, jwt_secret, self.jwt_ttl_secs);
        Ok((user, token))
    }
...
mod tests {
    ...
    use super::*;
    use crate::modules::user::query::UserQueryService;
    use sqlx::SqlitePool;
    ...
    async fn setup() -> (UserService, UserQueryService) {
        ...
        (UserService::new(pool, 864000), qsvc)
    }
    ...
    async fn change_password_short_new() {
        ...
        let (user, _) = svc
            ...
            .unwrap();
        let err = svc
            .change_password(user.id, "realPass", "ab")
            .await
            .unwrap_err();
        assert!(matches!(err, ServiceError::InvalidInput(_)));
    }
