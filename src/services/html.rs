...
impl HtmlService {
    ...
    pub async fn get_ontos_for_html(&self) -> Result<Vec<onto::Model>, String> {
        self.onto_repository.find_all()
            .await
            .map_err(|e| format!("获取本体列表失败: {}", e))
    }

    /// 获取本体详情用于HTML展示
    pub async fn get_onto_detail_for_html(&self, id: i32) -> Result<(onto::Model, Vec<signifier_signified::Model>, Vec<signifier_signified::Model>), String> {
        // 获取本体信息
        let onto = self.onto_repository.find_by_id(id)
            .await
            .map_err(|e| format!("获取本体失败: {}", e))?
            .ok_or_else(|| format!("本体 ID {} 不存在", id))?;

        // 获取作为能指的关系
        let signifiers = self.sign_repository.find_by_signifier_id(id)
            .await
            .map_err(|e| format!("获取能指关系失败: {}", e))?;

        // 获取作为所指的关系
        let signifieds = self.sign_repository.find_by_signified_id(id)
            .await
    ...
    pub async fn get_signs_for_html(&self) -> Result<Vec<signifier_signified::Model>, String> {
        self.sign_repository.find_all()
            .await
            .map_err(|e| format!("获取符号关系列表失败: {}", e))
    }

    /// 获取符号关系详情用于HTML展示
    pub async fn get_sign_detail_for_html(&self, id: i32) -> Result<signifier_signified::Model, String> {
        self.sign_repository.find_by_id(id)
            .await
    ...
    /// 获取能指相关的符号关系用于HTML展示
    pub async fn get_signs_by_signifier_for_html(&self, signifier_id: i32) -> Result<(onto::Model, Vec<signifier_signified::Model>), String> {
        // 获取本体信息
        let onto = self.onto_repository.find_by_id(signifier_id)
            .await
            .map_err(|e| format!("获取能指本体失败: {}", e))?
            .ok_or_else(|| format!("能指本体 ID {} 不存在", signifier_id))?;

        // 获取作为能指的关系
        let signs = self.sign_repository.find_by_signifier_id(signifier_id)
            .await
            .map_err(|e| format!("获取能指关系失败: {}", e))?;
    ...
    /// 获取所指相关的符号关系用于HTML展示
    pub async fn get_signs_by_signified_for_html(&self, signified_id: i32) -> Result<(onto::Model, Vec<signifier_signified::Model>), String> {
        // 获取本体信息
        let onto = self.onto_repository.find_by_id(signified_id)
            .await
            .map_err(|e| format!("获取所指本体失败: {}", e))?
            .ok_or_else(|| format!("所指本体 ID {} 不存在", signified_id))?;

        // 获取作为所指的关系
        let signs = self.sign_repository.find_by_signified_id(signified_id)
            .await
            .map_err(|e| format!("获取所指关系失败: {}", e))?;
    ...
    pub async fn validate_onto_id(&self, id: i32) -> Result<bool, String> {
        self.onto_repository.find_by_id(id)
            .await
            .map(|opt| opt.is_some())
