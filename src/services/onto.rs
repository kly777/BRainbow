...
impl OntoService {
    ...
    pub async fn create_onto(&self, name: String) -> Result<onto::Model, String> {
        ...
        // 创建本体
        self.onto_repository.create(name)
            .await
            .map_err(|e| format!("创建本体失败: {}", e))
    }

    /// 更新本体
    pub async fn update_onto(&self, id: i32, name: Option<String>) -> Result<Option<onto::Model>, String> {
        // 验证名称是否为空（如果提供了名称）
        ...
        // 更新本体
        self.onto_repository.update(id, name)
            .await
    ...
    pub async fn delete_onto(&self, id: i32) -> Result<(), String> {
        // 检查本体是否存在
        if self.onto_repository.find_by_id(id).await
        ...
        // 删除本体
        self.onto_repository.delete(id)
            .await
