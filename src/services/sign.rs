...
impl SignService {
    ...
    /// 根据ID获取符号关系
    pub async fn get_sign_by_id(&self, id: i32) -> Result<Option<signifier_signified::Model>, String> {
    ...
    pub async fn create_sign(
        ...
        // 验证能指和所指是否存在
        if !self.onto_repository.find_by_id(signifier_id).await
            .map_err(|e| format!("验证能指本体失败: {}", e))?
            .is_some() {
            return Err(format!("能指本体 ID {} 不存在", signifier_id));
        }

        if !self.onto_repository.find_by_id(signified_id).await
        ...
        // 检查关系是否已存在
        if self.sign_repository.exists(signifier_id, signified_id).await
            .map_err(|e| format!("检查符号关系存在性失败: {}", e))? {
            return Err(format!("能指 {} 到所指 {} 的关系已存在", signifier_id, signified_id));
        }
        ...
        // 创建符号关系
        self.sign_repository.create(signifier_id, signified_id, weight, relation_type)
            .await
    ...
    pub async fn delete_sign(&self, id: i32) -> Result<(), String> {
        // 检查符号关系是否存在
        if self.sign_repository.find_by_id(id).await
            .map_err(|e| format!("检查符号关系存在性失败: {}", e))?
            .is_none() {
        ...
        // 删除符号关系
        self.sign_repository.delete(id)
            .await
    ...
    /// 根据能指ID获取符号关系
    pub async fn get_signs_by_signifier(&self, signifier_id: i32) -> Result<Vec<signifier_signified::Model>, String> {
        // 验证能指是否存在
        if !self.onto_repository.find_by_id(signifier_id).await
            .map_err(|e| format!("验证能指本体失败: {}", e))?
            .is_some() {
        ...
        self.sign_repository.find_by_signifier_id(signifier_id)
            .await
            .map_err(|e| format!("获取能指相关关系失败: {}", e))
    }

    /// 根据所指ID获取符号关系
    pub async fn get_signs_by_signified(&self, signified_id: i32) -> Result<Vec<signifier_signified::Model>, String> {
        // 验证所指是否存在
        if !self.onto_repository.find_by_id(signified_id).await
            .map_err(|e| format!("验证所指本体失败: {}", e))?
            .is_some() {
        ...
        self.sign_repository.find_by_signified_id(signified_id)
            .await
