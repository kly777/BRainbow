...
export const getSignsBySignifier = (signifier: string): Effect.Effect<readonly Sign[], ApiErrorType> =>
    request(`/sign/signifier/${encodeURIComponent(signifier)}`, Schema.Array(SignSchema), {});

export const getSignsBySignified = (signified: string): Effect.Effect<readonly Sign[], ApiErrorType> =>
    request(`/sign/signified/${encodeURIComponent(signified)}`, Schema.Array(SignSchema), {});
