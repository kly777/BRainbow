...
export default function EditTaskModal(props: EditTaskModalProps) {
	...
	// ── Tab 切换 ──
	const [activeTab, setActiveTab] = createSignal<"basic" | "time" | "deps">("basic");
	...
	const handleAddTimeWindow = async () => {
		...
		try {
			...
			setDepError(msg.includes("planned_outside") ? "计划时间必须在可进行时间窗口内" : msg.includes("slot_overlap") ? "时间段与现有时间段重叠" : `创建失败: ${msg}`);
		}
	};
	...
	const handleAddDependency = async () => {
		...
		try {
			...
			const msg = getErrorMessage(e);
			setDepError(msg.includes("Circular") ? "不能形成循环依赖" : msg.includes("self") ? "不能依赖自己" : `添加失败: ${msg}`);
		}
	};
	...
	return (
		<Modal
			...
			actions={
				<>
					<button type="button" onClick={props.onClose} class={styles.cancelBtn}>取消</button>
					<button type="button" onClick={handleSave} class={styles.saveBtn}>保存</button>
				</>
			}
		>
			<div class={styles.editModal}>
				...
				<div class={styles.tabBar}>
					<button type="button" class={`${styles.tab} ${activeTab() === "basic" ? styles.tabActive : ""}`} onClick={() => setActiveTab("basic")}>
						基本信息
					</button>
					<button type="button" class={`${styles.tab} ${activeTab() === "time" ? styles.tabActive : ""}`} onClick={() => setActiveTab("time")}>
						时间段
						<Show when={feasibleWindows().length + plannedWindows().length > 0}>
							<span class={styles.tabCount}>{feasibleWindows().length + plannedWindows().length}</span>
						</Show>
					</button>
					<button type="button" class={`${styles.tab} ${activeTab() === "deps" ? styles.tabActive : ""}`} onClick={() => setActiveTab("deps")}>
						依赖关系
						<Show when={depIds().length > 0}>
							<span class={styles.tabCount}>{depIds().length}</span>
						</Show>
					</button>
				</div>
					...
					<div class={styles.tabContent}>
						...
						<div class={styles.presets}>
							...
							<For each={presetTimeSlots}>
								{(p) => (
									<button type="button" class={styles.presetBtn} onClick={() => applyPreset(p)}>
										{p.label}
									</button>
								)}
							</For>
								...
								<For each={feasibleWindows()}>
									{(tw) => (
										<div class={styles.timeItem}>
											<span class={styles.timeItemText}>🟢 {formatDateTime(tw.start_time)} ~ {formatDateTime(tw.end_time)}</span>
											<button type="button" onClick={() => handleDeleteTimeWindow(tw.id, "feasible")} class={styles.timeDelete}>×</button>
										</div>
									)}
								</For>
							</div>
							...
							<div class={styles.timeList}>
								<For each={plannedWindows()}>
									{(tw) => (
										<div class={styles.timeItem}>
											<span class={styles.timeItemText}>🔵 {formatDateTime(tw.start_time)} ~ {formatDateTime(tw.end_time)}</span>
											<button type="button" onClick={() => handleDeleteTimeWindow(tw.id, "planned")} class={styles.timeDelete}>×</button>
										</div>
									)}
								</For>
							</div>
									...
									{(t) => (
										<div class={styles.depItem}>
											<div class={styles.depInfo}>
												...
												<span class={`${styles.depStatus} ${styles[`depStatus_${t.status || "backlog"}`]}`}>
													{t.status || "backlog"}
												</span>
											</div>
											<button type="button" onClick={() => handleRemoveDependency(t.id)} class={styles.depRemove} title="移除依赖">
												×
											</button>
										</div>
									)}
						...
						<div class={styles.addDepBlock}>
							...
							<div class={styles.addDepRow}>
								<select
									value={newDepId() ?? ""}
									onChange={(e) => {
										setNewDepId(e.currentTarget.value ? parseInt(e.currentTarget.value, 10) : undefined);
										setDepError("");
									}}
								...
								<button type="button" onClick={handleAddDependency} disabled={!newDepId()} class={styles.addBtn}>
									+ 添加
								</button>
							</div>
							<Show when={depError()}>
