...
export default function MemTable(props: Props) {
	return (
		<Show
			when={!props.loading}
					...
					<For each={props.mems}>
						{(mem) => {
							...
							return (
								<tr
									class={
										props.detailId === mem.id ? styles.rowActive : styles.row
									}
								>
			...
			<Show when={props.pageMeta.total_pages > 1}>
				<div class={styles.pagination}>
					...
					<span class={styles.pageInfo}>
						{props.pageMeta.page} / {props.pageMeta.total_pages}
						（共{props.pageMeta.total}条）
					</span>
					<button
						type="button"
						class={styles.pageBtn}
