...
/** 拓扑排序 + 分层，返回节点坐标和边 */
function layout(nodes: readonly DagNode[], edges: readonly DagEdge[]): { nodes: LayoutNode[]; edges: LayoutEdge[] } {
	...
	const layoutNodes: LayoutNode[] = nodes.map((n) => {
		...
		return { id: n.id, title: n.title, status: n.status, x: pos.x, y: pos.y };
	});
...
export default function TaskDag() {
	...
	const [dagData] = createResource(
		...
		async ({ taskId, depth: d }) => {
			try {
				return await Effect.runPromise(getDag(taskId, d));
			} catch (e) {
	...
	const [allTasks] = createResource(async () => {
		try {
			const r = await Effect.runPromise(getAllTasks());
			return [...r.items];
		} catch {
			return [];
		}
	...
	const render = () => {
		...
		drawGraph(ctx, lay, scale(), autoOffX + offset().x, autoOffY + offset().y, hoveredId());
	};

	createEffect(() => { layoutData(); render(); });
	...
	const onCanvasHover = (e: MouseEvent) => {
		const canvas = canvasRef;
		const lay = layoutData();
		if (!canvas || !lay) { setHoveredId(null); return; }
		...
		<div class={styles.dagContainer}>
			<div class={styles.dagToolbar}>
				...
				<div class={styles.dagControls}>
					<TagSelector tasks={allTasks() || []} value={taskFilter()} onChange={setTaskFilter} />
					<select
						class={styles.taskSelect}
						value={depth()}
					...
					<button type="button" class={styles.zoomBtn} onClick={() => setScale((s) => Math.min(3, s + 0.2))} title="放大">
						+
					</button>
					<button type="button" class={styles.zoomBtn} onClick={() => setScale((s) => Math.max(0.3, s - 0.2))} title="缩小">
						−
					</button>
					<button type="button" class={styles.zoomBtn} onClick={() => { setScale(1); setOffset({ x: 0, y: 0 }); }} title="重置">
						↺
					</button>
				</div>
			</div>
			<div class={styles.canvasWrap}>
				<Show
					when={(() => { const ld = layoutData(); return ld && ld.nodes.length > 0; })()}
					fallback={
						<div class={styles.emptyHint}>
							<p>暂无依赖关系</p>
							<p class={styles.hintSub}>在任务详情页为任务添加依赖后，可在此处查看依赖图</p>
						</div>
					}
				>
					<canvas
