...
import { getAiSettings, setAiSettings, resetAiSettings } from "../../../lib/ai-settings.ts";
...
export default function AiSettingsModal(props: AiSettingsModalProps) {
	...
	const [mnemonicPrompt, setMnemonicPrompt] = createSignal(saved.mnemonicPrompt);
		...
		<Modal isOpen={props.isOpen} onClose={props.onClose} title="AI 设置">
			<div style={{ display: "flex", "flex-direction": "column", gap: "16px", "min-width": "480px" }}>
				...
				<div class={styles.field}>
					<label class={styles.fieldLabel} for="ai-endpoint">API 地址</label>
					<input
						id="ai-endpoint"
						type="url"
						value={endpoint()}
				...
				{/* API Key */}
				<div class={styles.field}>
					<label class={styles.fieldLabel} for="ai-key">API Key</label>
					<input
						id="ai-key"
						type="password"
						value={apiKey()}
				...
				{/* 模型 */}
				<div class={styles.field}>
					<label class={styles.fieldLabel} for="ai-model">模型</label>
					<input
						id="ai-model"
						type="text"
						value={model()}
				...
				<div style={{ display: "flex", "align-items": "center", gap: "8px" }}>
					...
					<Show when={testStatus() === "fail"}>
						<span style={{ color: "#dc2626", "font-size": "13px" }}>{testMsg()}</span>
					</Show>
				</div>
				...
				<div class={styles.field}>
					<label class={styles.fieldLabel} for="ai-prompt">
						助记生成提示词
						<span style={{ "font-weight": "normal", color: "#6b7280", "margin-left": "8px", "font-size": "12px" }}>
							（可用 {'{cue}'}、{'{target}'} 作为占位符）
						</span>
					</label>
					<textarea
