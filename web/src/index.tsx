...
function AuthStatus() {
	...
	return (
		<div style={{ display: "flex", "align-items": "center", gap: "8px" }}>
			<Show when={auth().user}>
				<button type="button" onClick={handleLogout} style={{ ...btnStyle, background: "#6b7280" }}>退出</button>
			</Show>
			<Show when={!auth().user}>
				<button type="button" onClick={() => { setShowForm(!showForm()); setIsRegister(false); }} style={btnStyle}>
					登录
				</button>
				<button type="button" onClick={() => { setShowForm(!showForm()); setIsRegister(true); }} style={{ ...btnStyle, background: "#8b5cf6" }}>
					注册
				</button>
			</Show>
			<Show when={showForm()}>
				<button type="button" style={{ ...overlayStyle, border: "none" }} onClick={() => setShowForm(false)} onKeyDown={e => { if (e.key === "Escape") setShowForm(false); }}>
					<form onSubmit={handleSubmit} style={formStyle} onClick={e => e.stopPropagation()} onKeyDown={e => { if (e.key === "Escape") setShowForm(false); }}>
						<h3 style={{ margin: 0 }}>{isRegister() ? "注册" : "登录"}</h3>
						{error() && <p style={{ color: "red", "font-size": "13px" }}>{error()}</p>}
						<input placeholder="用户名" value={name()} onInput={e => setName(e.currentTarget.value)} style={inputStyle} />
						<input type="password" placeholder="密码" value={password()} onInput={e => setPassword(e.currentTarget.value)} style={inputStyle} />
						<button type="submit" style={{ ...btnStyle, width: "100%" }}>{isRegister() ? "注册" : "登录"}</button>
						<button type="button" onClick={() => setShowForm(false)} style={{ ...btnStyle, width: "100%", background: "#6b7280" }}>取消</button>
					</form>
				</button>
			</Show>
		</div>
...
const inputStyle = {
	padding: "10px", border: "1px solid #d1d5db", "border-radius": "6px", "font-size": "14px",
};
