...
describe("bookmark API", () => {
	...
	it("获取书签列表", async () => {
		...
		expect(mockCachedRequest).toHaveBeenCalledWith("/bookmarks?page=1&page_size=20&tag=%E6%B5%8B%E8%AF%95");
	...
	it("创建书签", async () => {
		...
		mockDomains.bookmarks.invalidate.mockImplementationOnce((promise) => promise);
	...
	it("更新书签", async () => {
		...
		expect(mockPatch).toHaveBeenCalledWith("/bookmarks/1", { title: "更新后的书签" });
	...
	it("搜索书签", async () => {
		...
		// 验证cachedRequest被调用
		expect(mockCachedRequest).toHaveBeenCalledTimes(1);
		expect(mockCachedRequest).toHaveBeenCalledWith("/bookmarks/search?q=%E6%B5%8B%E8%AF%95&page=1&page_size=20&tag=%E6%A0%87%E7%AD%BE");
	...
	it("管理标签", async () => {
		// 测试管理标签
		const { cachedRequest, request, del, domains } = await import("@shared/api");
		const mockCachedRequest = vi.mocked(cachedRequest);
		...
		// 验证cachedRequest被调用
		expect(mockCachedRequest).toHaveBeenCalledTimes(1);
		expect(mockCachedRequest).toHaveBeenCalledWith("/bookmarks/tags?q=%E6%B5%8B%E8%AF%95");
