...
import { getCardsE, getCardE, createCardE, updateCardE, deleteCardE, searchCardsE } from "./api.ts";
...
describe("card API", () => {
	...
	it("获取卡片列表", async () => {
		...
		expect(mockCachedRequest).toHaveBeenCalledWith("/cards?page=1&page_size=20");
	...
	it("搜索卡片", async () => {
		...
		// 验证cachedRequest被调用
		expect(mockCachedRequest).toHaveBeenCalledTimes(1);
		expect(mockCachedRequest).toHaveBeenCalledWith("/cards/search?q=%E6%B5%8B%E8%AF%95&page=1&page_size=20");
