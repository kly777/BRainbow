...
import { uploadMedia, listMediaE, getMediaE, renameMediaE, deleteMediaE } from "./api.ts";
...
describe("media API", () => {
	...
	it("获取媒体列表", async () => {
		...
		// 调用获取媒体列表API
		const result = await listMediaE({ media_type: "image", page: 1, page_size: 20 });
		
		// 验证cachedRequest被调用
		expect(mockCachedRequest).toHaveBeenCalledTimes(1);
		expect(mockCachedRequest).toHaveBeenCalledWith("/media?media_type=image&page=1&page_size=20");
	...
	it("重命名媒体", async () => {
		...
		expect(mockPatch).toHaveBeenCalledWith("/media/abc123", { original_name: "新名称.jpg" });
