...
describe("reading API", () => {
	...
	it("上传文章", async () => {
		...
		expect(mockPost).toHaveBeenCalledWith("/reading", { title: "新文章", content: "新内容" });
	...
	it("标记单词状态", async () => {
		...
		// 验证post被调用
		expect(mockPost).toHaveBeenCalledTimes(1);
		expect(mockPost).toHaveBeenCalledWith("/reading/word/%E6%B5%8B%E8%AF%95", { status: "known" });
	...
	it("获取和更新文章笔记", async () => {
		...
		expect(mockPut).toHaveBeenCalledWith("/reading/1/notes", { notes: "新笔记" });
