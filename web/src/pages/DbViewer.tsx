  ...
  return (
    <div style={{ display: "flex", height: "100%", overflow: "hidden" }}>
      <div style={{
        ...
        padding: "10px", background: "var(--bg-input, #fef9f2)",
        "border-right": "1px solid var(--border, #f0e6d3)",
      }}>
        <div style={{ "font-weight": "600", padding: "6px 8px", "font-size": "13px", color: "#6b7280" }}>
          表列表
        </div>
        <For each={tables()}>
          {(t) => (
            <button
              ...
              onClick={() => loadTable(t)}
              style={{
                display: "block", width: "100%", padding: "8px 12px", cursor: "pointer", "font-size": "13px",
                "border-radius": "6px", color: "#374151", "text-align": "left",
      ...
      <div style={{ flex: "1", overflow: "auto", padding: "16px 20px" }}>
        {error() && (
          <div style={{ padding: "10px", background: "#fef2f2", color: "#dc2626", "border-radius": "6px", "font-size": "13px", "margin-bottom": "12px" }}>
            {error()}
          </div>
        )}
        {loading() && <div style={{ color: "#6b7280", padding: "12px" }}>加载中...</div>}
          ...
          <>
            ...
            <div style={{ "overflow-x": "auto" }}>
              <table style={{ "border-collapse": "collapse", width: "100%", "font-size": "13px" }}>
                    ...
                    <For each={columns()}>
                      {(c) => (
                        <th style={{
                          ...
                          <div style="font-size:10px;font-weight:400;color:#9ca3af">{c.col_type}</div>
                        </th>
                      )}
                    </For>
                  </tr>
                ...
                <tbody>
                  {rows().length === 0 && (
                    <tr><td colspan={columns().length} style={{ padding: "20px", color: "#9ca3af", "text-align": "center" }}>无数据</td></tr>
                  )}
                  <For each={rows()}>
                    {(row) => (
                      <tr>
