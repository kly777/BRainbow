...
function TaskSection() {
	const { tasks, loading, add, updateStatus, removeTask, updateTask, addSubTask } = useTasks();
	const [title, setTitle] = createSignal("");
	const [desc, setDesc] = createSignal("");

	const handleAdd = async () => {
		if (!title().trim()) return;
		const req: CreateTaskRequest = { title: title().trim(), description: desc().trim() || undefined };
		setTitle("");
		setDesc("");
		await add(req);
	};
	...
	return (
		<div class={styles.todoSection}>
			...
			<div class={styles.addTodoForm}>
				<input type="text" placeholder="输入新任务标题..." value={title()} onInput={(e) => setTitle(e.currentTarget.value)} class={styles.todoInput} onKeyPress={(e) => e.key === "Enter" && handleAdd()} />
				<textarea placeholder="任务描述（可选）..." value={desc()} onInput={(e) => setDesc(e.currentTarget.value)} class={styles.todoTextarea} />
				<div class={styles.formActions}>
					<button type="button" onClick={handleAdd} class={styles.addButton} disabled={!title().trim()}>添加任务</button>
				</div>
			</div>

			<Show when={tasks().length > 0} fallback={
				<div class={styles.emptyState}>
					<Show when={!loading()}><p>暂无待办事项</p><p class={styles.emptyHint}>添加您的第一个任务吧！</p></Show>
...
function CardSection() {
	...
	const load = async () => {
		...
		try {
			...
			const sorted = [...r.items].sort((a, b) => new Date(b.updated_at).getTime() - new Date(a.updated_at).getTime());
