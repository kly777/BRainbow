...
function TaskOverview() {
  const { tasks, loading, updateStatus, removeTask, updateTaskE, addSubTask } = useTasks();
    ...
    <section class={styles.dashboardSection}>
      <div class={styles.sectionHeader}>
        ...
        <div class={styles.sectionActions}>
          <A href="/t" class={styles.viewAllLink}>查看全部 →</A>
        </div>
      </div>
...
function CardOverview() {
  ...
  const [cards, { mutate, refetch }] = createResource(async (): Promise<CardData[]> => {
    ...
    <section class={styles.dashboardSection}>
      <div class={styles.sectionHeader}>
        ...
        <div class={styles.sectionActions}>
          ...
          <A href="/c" class={styles.createLink}>+ 新建</A>
        </div>
      </div>
      <AsyncView
        data={recentCards()}
