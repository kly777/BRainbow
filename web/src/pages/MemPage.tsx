...
export default function MemPage() {
    ...
    const rate = async (rating: number) => {
        ...
        lastAction = {
            id: it.id,
            undoData: { state: it.state, stability: it.stability, difficulty: it.difficulty, step_index: null, lapses: 0, leeched: false, due_at: it.due_at },
        };
    ...
    const bury = async () => {
        const it = item();
        if (!it) return;
        await Effect.runPromiseExit(buryMem(it.id));
        loadDue();
    };
