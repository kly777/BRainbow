...
impl Remote {
    ...
    pub fn send_stdin(&self, cmd: &str, content: &[u8], label: &str) -> Result<()> {
        ...
        let status = child
            .wait()
            .map_err(|e| Error::io("等待 ssh 结束", e))?;
        ...
        F: FnOnce(&mut Archive) -> Result<()>,
    {
        ...
        let built = build(&mut archive).and_then(|()| {
            let gz = archive.into_inner().map_err(|e| Error::io("收尾 tar 归档", e))?;
...
mod tests {
    ...
    fn glob_in_keeps_pattern_literal() {
        ...
        assert_eq!(glob_in("/opt/brb/backup", "db_*.db"), "'/opt/brb/backup'/db_*.db");
    ...
    fn path_of_returns_a_raw_value() {
        ...
        assert_eq!(path_of("/opt/brb/service", "brainbow"), "/opt/brb/service/brainbow");
